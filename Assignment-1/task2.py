...
def for_plot(seed=0, ALGO=Eps_Greedy, PROBS=[0.3, 0.5, 0.7], HORIZON=1000, i=0):
  HORIZON = 300
  np.random.seed(seed)
  ...
  for t in range(HORIZON):
    ...
    arm_pulled.append(arm_to_be_pulled)
    reward = bandit.pull(arm_to_be_pulled)
    algo_inst.get_reward(arm_index=arm_to_be_pulled, reward=reward)
  plt.plot(arm_pulled)
  plt.grid(linestyle=':')
  plt.fill_between(range(HORIZON), 0, arm_pulled, color='r', alpha=0.1)
  plt.savefig("plot_{}_{}".format(i, ALGO.__name__))
  plt.close()
...
def task2(algorithm, horizon, p1s, p2s, num_sims=50):
    ...
    for prob in probs:
        ...
        for_plot(1, algorithm, prob, horizon, i)
        i += 1
    return regrets
...
if __name__ == '__main__':
  ...
  plt.xlabel('p2')
  plt.ylabel('regrets')
  plt.title('UCB regrets vs p2')
  plt.savefig("Task-2A")
  plt.close()
  # Part B

  task2p1s = np.arange(0.1, 1.05, 0.05)
  task2p2s = np.arange(0, 0.95, 0.05)
  task2p1s[18] = 1
  regrets1 = task2(UCB, 30000, task2p1s, task2p2s)
  regrets2 = task2(KL_UCB, 30000, task2p1s, task2p2s)
  plt.plot(task2p2s,regrets1)
  plt.xlabel('p2')
  plt.ylabel('regrets')
  plt.title('regrets vs p2 in UCB algorithm')
  plt.savefig("Task-2B_UCB")
  plt.close()
  plt.plot(task2p2s, regrets2)
  plt.xlabel('p2' )
  plt.ylabel('regrets')
  plt.title('regrets vs p2 in KL_UCB algorithm')
  plt.savefig("Task-2B_KL_UCB")
  plt.close()
  print(regrets1)
  print(regrets2)
